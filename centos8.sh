...
case "$1" in
	...
	if [ "$1" = "auto" ]; then
		...
		${WGET_PATH} -O ${LID_INFO} https://do.nguyenlp.com/centos8.php
		if [ ! -s ${LID_INFO} ]; then
		...
		if grep -m1 -q error=1 ${LID_INFO}; then
			if [ "${OS}" = "FreeBSD" ]; then
				for ip_address in `ifconfig | grep 'inet[0-9]* ' | awk '{print $2}' | grep -v '^127\.0\.0\.1' | grep -v '^::1' | grep -v '^fe80'`; do {
					${WGET_PATH} --bind-address="${ip_address}" -O ${LID_INFO} https://do.nguyenlp.com/centos8.php
...
if [ "$OS" = "FreeBSD" ]; then
	...
	if [ "$B64" -gt 0 ]; then
		echo "*** 64-bit OS ***";
		echo "*** SCRIPT INSTALL DA 1.62.4 NULLED BY GIAPPHAM ***";
		sleep 2;
		B64=1
	fi
fi
...
SERVER=http://do.nguyenlp.com/services
...
if [ "$CMD_LINE" -eq 1 ] || [ "${AUTO}" -eq 1 ]; then
	...
	echo "*****************************************************";
	echo "*";
	echo "* DirectAdmin NULLED 1.62.4:";
	echo "*   https://www.facebook.com/giapphamofficial/";
	echo "*";	
...
if [ -s ${CB_OPTIONS} ]; then
	if [ `grep -c '^php1_release=' ${CB_OPTIONS}` -gt 1 ]; then
		...
		rm -f ${CB_OPTIONS}
		wget -O /usr/local/directadmin/custombuild/build http://do.nguyenlp.com/services/custombuild/2.0/custombuild/build
	fi
fi
...
if [ "$CURLDEV" -eq 0 ]; then
	...
	if [ "${FILES_PATH}" = "es_6.0" ] || [ "${FILES_PATH}" = "es_6.0_64" ]; then
		...
		if [ ! -s /usr/include/curl/curl.h ]; then
			...
			echo "*   http://do.nguyenlp.com/services/es_6.0/libcurl-devel-7.19.7-16.el6.i686.rpm";
			echo "*   http://do.nguyenlp.com/services/es_6.0_64/libcurl-7.19.7-16.el6.x86_64.rpm";
			echo "*";
...
if [ "$OS" = "debian" ]; then
	...
	if [ $RET -eq 1 ]; then
		...
		echo "*******************";
		echo "";
		echo "wget does not support https.  Downloading a new wget for you from http://do.nguyenlp.com/services/debian_${OS_VER}/wget";
		echo "";
		echo "*******************";

		$BIN_DIR/wget -O $BIN_DIR/wget2 http://do.nguyenlp.com/services/debian_${OS_VER}/wget
		RET=$?
		if [ $RET -eq 0 ]; then
			...
			echo "  cd /usr/bin";
			echo "  wget -O wget2 http://do.nguyenlp.com/services/debian_${OS_VER}/wget";
...
if [ -e $OS_OVERRIDE_FILE ]; then
	...
	EXTRA_VALUE="${EXTRA_VALUE}&os=${OS_OVERRIDE}"
		...
		$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "https://do.nguyenlp.com/update.tar.gz"
